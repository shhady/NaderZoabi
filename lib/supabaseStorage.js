...
export async function uploadFile(file, metadata, bucketName = 'documents') {
  try {
...
export async function getSignedUrl(filePath, bucketName = 'documents') {
  try {
    const { data, error } = await supabase.storage
      .from(bucketName)
...
export async function deleteFile(fileId, filePath, userId, isAdmin, bucketName = 'documents') {
  try {
    // Check permissions
